...
export function checkPagesLayout(input: PagesLayoutInput): string[] {
  ...
  const raise = input.raiseScript ?? "";
  if (raise && /pages["\s,]+deploy["\s,]+frontend\/dist/.test(raise)) {
    problems.push(
      "staging:raise.ts passes a positional assets directory to wrangler pages deploy; drop it so functions/ and D1 actually ship.",
    );
  }
...
if (isDirectRun) {
  ...
  const problems = checkPagesLayout({
    ...
    raiseScript: readOrNull(resolve(root, "scripts/staging-raise.ts")),
  });
