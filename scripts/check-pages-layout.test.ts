...
describe("checkPagesLayout", () => {
  ...
  it("rejects staging:raise passing a positional Pages assets directory", () => {
    const problems = checkPagesLayout({
      ...good,
      raiseScript: `run("pages:deploy:preview", "npx", [
      "wrangler",
      "pages",
      "deploy",
      "frontend/dist",
    ...
    expect(problems.join(" ")).toMatch(/staging:raise/);
  });
